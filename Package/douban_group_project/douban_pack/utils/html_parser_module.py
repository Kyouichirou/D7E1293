...
class Parser:
    ...
    def topic_detail(self, dom, url_id: str) -> dict:
        ...
        if js := dom.find('script', attrs={'type': 'application/ld+json'}):
            ...
            # "\" 符号可能导致转为json出现错误
            objs = json.loads(content, strict=False)
            info['title'] = objs['name']
            info['text'] = objs['text'].strip()
            info['commentCount'] = int(objs['commentCount'] or '0')
            info['dateCreated'] = convert_date(objs['dateCreated'])
            if tmp := objs.get('interactionStatistic', False):
